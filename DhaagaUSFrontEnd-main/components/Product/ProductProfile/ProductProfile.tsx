...
import BusinessHoursModal from "../../ModalOverlay/BusinessHoursModal";
import AddReviewModal from "../../ModalOverlay/AddReviewModal";
import { BusinessDay, ProductDetail } from "@/Models/ListData";
...
import ErrorParticular from "../../ErrorPage/ErrorParticular";

import styles from "../../../styles/ProductProfile.module.css";
...
const ProductProfile = (props: { productDetail: ProductDetail }) => {
  const { productDetail } = props;
  const { product, isFavorite } = productDetail;
  const [firstRenderDone, setFirstRenderDone] = useState(false);
  const [businessHoursModalShow, setBusinessHoursModalShow] = useState(false);
  ...
  const [googleReviews, setGoogleReviews] = useState<
    [[string], [string]][] | []
  >([]);
  const [resultOfAddingReview, setResultOfAddingReview] = useState<null | {
    success?: string;
    error?: string;
  }>(null);
  ...
  const {
    isFavoriteProduct,
    requestResponse,
    show: showFavProductResponseToast,
    hideToast,
    addAsFavorite,
    removeAsFavorite,
    setIsFavorite,
  ...
  useEffect(() => {
    ...
    if (product.business_hours) {
      ...
      if (isDayToday) {
        if (
          product.business_hours[isDayToday][2] &&
          product.business_hours[isDayToday][2].closed
        ) {
          timingsOfToday = product.business_hours[isDayToday][2].closed!;
        } else if (
          product.business_hours[isDayToday][2] &&
          product.business_hours[isDayToday][2].open
        ) {
          timingsOfToday = product.business_hours[isDayToday][2].open!;
        } else if (
          product.business_hours[isDayToday].length === 2 &&
          product.business_hours[isDayToday][0].open &&
          product.business_hours[isDayToday][0].close &&
          product.business_hours[isDayToday][1].open &&
          product.business_hours[isDayToday][1].close
        ) {
          timingsOfToday = `${changeTimeToTwelveHourFormat(
            (product.business_hours[isDayToday][0] as BusinessDay).open
          )}-${changeTimeToTwelveHourFormat(
            (product.business_hours[isDayToday][0] as BusinessDay).close
          )} \u00A0${changeTimeToTwelveHourFormat(
            (product.business_hours[isDayToday][1] as BusinessDay).open
          )}-${changeTimeToTwelveHourFormat(
            (product.business_hours[isDayToday][1] as BusinessDay).close
          )}`;
        } else if (
          product.business_hours[isDayToday].length === 2 &&
          product.business_hours[isDayToday][0].open &&
  ...
  useEffect(() => {
    ...
    setIsProductFavorite(isFavoriteProduct!);
  }, [isFavoriteProduct]);

  useEffect(() => {
    if (resultOfAddingReview) {
      setShow(true);
    }
  }, [resultOfAddingReview]);

  useEffect(() => {
    let reviewText: any = [];

    for (let key of Object.keys(product)) {
      if (key.startsWith("Review ") && (product[key] as string).trim() !== "") {
        reviewText.push([[key], [product[key] as string]]);
      }
    }
    setGoogleReviews(reviewText);
  }, [product]);

  const AddOrRemoveAsFavProductHandler = (identifier: string) => {
    if (identifier === "fav") {
      addAsFavorite(product._id);
    } else {
      removeAsFavorite(product._id, true);
    }
  };

  const getResultOfAddingReview = (result: {
    success?: string;
    error?: string;
  }) => {
    setResultOfAddingReview(result);
  };

  return (
    <>
      {resultOfAddingReview && resultOfAddingReview.error && show && (
        <ToastModal
          variant="danger"
          header="Error!"
          body={resultOfAddingReview.error}
          show={show}
          onClose={() => setShow(false)}
        />
      )}
      {resultOfAddingReview && resultOfAddingReview.success && show && (
        <ToastModal
          variant="success"
          header="Success"
          body={resultOfAddingReview.success}
          show={show}
          onClose={() => setShow(false)}
        />
      )}
      ...
      <BusinessHoursModal
        ...
        item={product.business_hours}
        show={businessHoursModalShow}
        onHide={() => setBusinessHoursModalShow(false)}
      />
      <AddReviewModal
        show={addReviewModalShow}
        onHide={() => setAddReviewModalShow(false)}
        onResult={getResultOfAddingReview}
      />
      <div
      ...
      >
        ...
        >
          <div className={styles.head}>Business Profile</div> */}

        {/* Edit Button */}
        {/* {user?.uid === product.owner_id && (
            <button
        ...
        <Header
          showEditButton
          isAuthorizedForEditButton={user?.uid === product.owner_id}
          headerText="Business Profile"
        />

        {/* <div className={`wrapper d-flex flex-column ${styles.details}`}>
          <ReviewsCountBadge  avgRating={product.avgRating} totalReviews={product.reviewCount} />
          <div
          ...
          >
            <li>
              ...
              <div className="d-flex" style={{ gap: 8 }}>
                ...
                <div className={styles["reach-text"]}>
            ...
            <li className="w-100">
              ...
              <div className="d-flex w-100" style={{ gap: 8 }}>
                ...
                {isBusinessHours(product.business_hours) ? (
                  <div
        ...
        <ProductDetails
          ...
          AddOrRemoveAsFavProductHandler={AddOrRemoveAsFavProductHandler}
          setBusinessHoursModalShow={setBusinessHoursModalShow}
        />
        <ProductImagesCarousel images={product.photos} />
        <Reviews
          reviewsHeader="Google Reviews"
          googleReviews={googleReviews}
          googleReviewsCount={product.reviews}
        />
        <ErrorBoundary FallbackComponent={ErrorParticular}>
          <Reviews
            reviewsHeader="Dhaaga Reviews"
            product={{
              Reviews: product.Reviews,
              dhaagaReviewCount: product.dhaagaReviewCount,
              hasReviewedProduct: productDetail.hasReviewedProduct,
            }}
            isAddReviewRequestSucceed={
              resultOfAddingReview?.success ? true : false
            }
            setAddReviewModalShow={() => setAddReviewModalShow(true)}
          />
        </ErrorBoundary>
      </div>
    </>
